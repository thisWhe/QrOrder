(function () {
    ...
    async function loadDashboard(state) {
        ...
        try {
            ...
            if (state.soundEnabled && hasNewCall) {
                playWaiterCallAlert(state);
            } else if (state.soundEnabled && hasNewOrder) {
                playReadyOrderAlert(state);
            }
        } catch (error) {
    ...
    function playReadyOrderAlert(state) {
        try {
            const context = getActiveAudioContext(state);
            if (!context) return;
    ...
    function playWaiterCallAlert(state) {
        try {
            const context = getActiveAudioContext(state);
            if (!context) return;

            playTone(context, 740, 0.00, 0.16, 0.26, "square");
            playTone(context, 980, 0.20, 0.16, 0.26, "square");
            playTone(context, 740, 0.40, 0.20, 0.28, "square");
        } catch {
            // Ses uyarisi yardimci ozellik; hata servis ekranini bozmamali.
        }
    }

    function getActiveAudioContext(state) {
        const AudioContextType = window.AudioContext || window.webkitAudioContext;
        if (!AudioContextType) return null;

        state.audioContext = state.audioContext || new AudioContextType();
        return state.audioContext.state === "suspended" ? null : state.audioContext;
    }

    function playTone(context, frequency, delay, duration, volume = 0.18, waveType = "sine") {
    ...
    function render(state) {
        ...
        titleWrap.innerHTML = `<h1 class="kitchen-title">Servis Ekrani</h1><div class="kitchen-meta"><span class="status-dot ${state.realtimeConnected ? "connected" : ""}"></span><span>${realtimeLabel}</span><span>${state.info || ""}</span></div>`;
